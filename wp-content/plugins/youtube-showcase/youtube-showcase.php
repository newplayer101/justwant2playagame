...
if (!class_exists('Youtube_Showcase')):
	...
	final class Youtube_Showcase {
		...
		private function define_constants() {
			define('YT_SCASE_COM_VERSION', '2.3.0');
		...
		public function load_plugin_textdomain() {
			$locale = apply_filters('plugin_locale', get_locale() , 'yt-scase-com');
			$mofile = sprintf('%1$s-%2$s.mo', 'yt-scase-com', $locale);
			$mofile_shared = sprintf('%1$s-emd-plugins-%2$s.mo', 'yt-scase-com', $locale);
			...
			foreach ($lang_file_list as $lang_key => $lang_file) {
				...
				$globalmo = WP_LANG_DIR . '/yt-scase-com/' . $lang_file;
